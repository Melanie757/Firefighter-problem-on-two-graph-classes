...
//--------------------------------------------------------------------------------------------------------
  ...
  //case 1)
    ...
    if(max_deg >= 3) {
      ...
      solution = part_solution(g, pos, adjlist_s, uc);
    ...
    //subcases
      ...
      pvec penalty;
      const auto pen = std::minmax(p,q);
      penalty.emplace_back(pen);

      solution = part_solution(g, pos, adjlist_s, uc, penalty);
    ...
    //else if(max_deg == 1) {
      ...
      for(int i=0; i<Y.size(); ++i) {
        ...
        if(adj_y[0] != adj_w[0]) {
          if(distinct.empty()) {
            distinct.emplace(n, adj_w[0]);
          }
          else{
            if(distinct.begin()->second != adj_w[0]){
              distinct.emplace(n, adj_w[0]);
            }
            else{
              continue;
      ...
      //1d)
        ...
        const int & p     = adj_y[0];
        Vertex    & v     = g.at(distinct.begin()->first);
      ...
      //1e) -> pretend to be in case 3)
        ...
        for(const auto & n : adjlist_s){
          ...
          if(xz.first == X.end()) {
            const auto pen = std::minmax(n,adj_y[0]);
            penalty.emplace_back(pen);
            continue;
          }
  ...
  for(int i=0; i<adjlist_s.size(); ++i) {
    ...
    if(max.numlinks() == 2) {
      ...
      if(cont) {
        // add pair (p,q)
        const auto pen = std::minmax(adj_max[0], adj_max[1]);
        penalty.emplace_back(pen);
      }
    }
  }
  ...
  solution = part_solution(g, pos, adjlist_s, uc, penalty, bonus);
  //third vertex to protect?
  return solution;
}
