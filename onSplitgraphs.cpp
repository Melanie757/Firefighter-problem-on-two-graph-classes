		...
		*/
		...
		for (int i=0; i<del2; ++i) {
			if (pairbonus[i] == 0) {
				...
				//find the pair of vertices related to pairbonus[i]
				const int c1 = i % delimiter;
				const int c2 = i / delimiter;

				/*
				std::cout << "c_1, c_2: " << c1 << ", " << c2 << std::endl;
				for(int i=0; i<adjlist_s.size(); ++i) {std::cout << adjlist_s[i] << ", ";}
				std::cout << std::endl;
